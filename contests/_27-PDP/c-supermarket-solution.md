...
Μπορούμε να κρατάμε ποια αντικείμενα έχουμε μετακινήσει σε ένα πίνακα $$\texttt{moved}[i]$$, όπου $$\texttt{moved}[i] = \texttt{false}$$ αν δεν μετακινήσαμε το αντικείμενο και $$\texttt{moved}[i] = \texttt{true}$$ αν το μετακινήσαμε στο τέλος του ιμάντα. Σε κώδικα μπορούμε να το γράψουμε κάπως έτσι:

{% include code.md solution_name='supermarket_brute_force.cc' start=36 end=53 %}

Η συνάρτηση $$\texttt{calculate_total_score}$$ πρέπει να υπολογίσει την τελική θέση κάθε αντικειμένου και να προσθέσει την αξία του αν η θέση είναι πολλαπλάσιο του $$K$$.
...
Το μόνο που μας μένει είναι να καλέσουμε την συνάρτηση $$\texttt{iterate(1, 0)}$$.
...
Παρατηρώντας ότι σε κάθε επανάληψη του $$i$$ χρησιμοποιούμε μόνο τιμές της μορφής $$\texttt{dp}[i-1][\cdot]$$, μπορούμε να μειώσουμε την μνήμη σε $$\mathcal{O}(M)$$ (δείτε τον κώδικα [εδώ]({% include link_to_source.md solution_name='supermarket_dp_mem.cc' %})).
