...
Αν έχουμε τον πίνακα $$Z$$ έτοιμο, μπορούμε να απαντάμε σε ερωτήματα αποδοτικά αν 
...
Χρησιμοποιώντας την *upper_bound*, βρίσκουμε 
το αριστερότερο νησί το οποίο ξεπερνά τα $$b$$ 
εισιτήρια που έχουμε στη διάθεση μας και πρέπει να σταματήσουμε το ταξίδι καθώς 
δεν μπορούμε να το ξεπεράσουμε. 
Το νησί αυτό θα έχει τιμή $$1$$ ή θα είναι το τελευταίο νησί $$N$$.
Κάθε ερώτημα θα απαντιέται σε χρόνο τάξης $$\mathcal{O}(log{N})$$ λόγω της δυαδικής 
αναζήτησης στα $$N$$ στοιχεία του πίνακα $$PS$$.

Το μειονέκτημα των prefix sum είναι ότι δεν είναι αποδοτικά στις ενημερώσεις τιμών 
και πρέπει να επανυπολογίζονται. Στο subtask αυτό όμως, όλες οι ανακοινώσεις που 
...
Η τεχνική που θα χρησιμοποιήσουμε αποθηκεύει μόνο την αρχή και το τέλος κάθε διαστήματος 
...
όλα τα ερωτήματα.
Κατά την προσπέλαση του πίνακα συμβάντων, θα χρειαστεί να κρατάμε ένα *set* με τις ενεργές ανακοινώσεις, 
ώστε να ξέρουμε ανα πάσα στιγμή ποιά είναι η πιό πρόσφατη. 
Η προσπέλαση του πίνακα συμβάντων θα χρειαστεί χρόνο 
της τάξης $$\mathcal{O}(Q \cdot log{Q} + N)$$ καθώς θα είσαγει και θα διαγράψει έως $$Q$$ στοιχεία στο *set* και 
...
Η δομή *segment tree* αποτελείται από ένα ισοροπημένο δυαδικό δέντρο με $$N$$ φύλλα και ύψος $$\ceil{log{N}}$$. 
...
Κάθε αλλαγή ενός στοιχείου του $$Z$$ (δηλαδή ενός φύλλου του *segment tree*) απαιτεί μόνο $$log{N}$$ αλλαγές στους κόμβους προγόνους του φύλλου.
Στο συγκεκριμένο *segment tree* υπολογίζουμε αθροίσματα διαστημάτων (range sums)που θα χρησιμοποιήσουμε στη λύση του subtask, 
...
το νησί $$1$$ στο $$a$$ και τα προσθέτουμε στο $$b$$. Με τον τρόπο αυτό μετασχηματίζουμε το ερώτημα που μας δίνεται και αντί για νησί αφετηρία 
να έχουμε το $$a$$, έχουμε το $$1$$. Κάνουμε δυαδική αναζήτηση στα νησιά μέχρι να βρούμε σε ποιό νησί 
...
δέντρο αναζήτησης (BST) άρα εμπεριέχει τη δυαδική αναζήτηση. Αν είμαστε σε έναν κόμβο $$si$$ που χρειάζεται $$st[si]$$ εισιτήρια, 
...
καθώς προσπεράσαμε τα νησιά του αριστερού απογόνου).
