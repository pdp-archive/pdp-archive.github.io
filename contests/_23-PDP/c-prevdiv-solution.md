...
**Παρατήρηση 1:** Αν έχουμε ελέγξει ότι $$A_1, \ldots A_{i-1}$$ διαιρούνε τον $$A_i$$, τότε όταν ελέγχουμε τον $$A_j$$ για $$j > i$$, δεν χρειάζεται να εξετάσουμε τους $$A_1, \ldots A_{i-1}$$, ότι $$A_i$$ διαιρεί τον $$A_j$$ (και τους μετέπειτα).

Αυτό από μόνο του δεν μας γλιτώνει πολλούς από τους ελέγχους γιατί μπορεί να έχουμε τον πίνακα $$2, 3, 3, 3, 3, 3, 3, 5, 6, 6, 6, 6$$. Άρα για τα $$6$$, ο πιο πρόσφατος αριθμός που διαιρείται από όλους τους προηγούμενούς του είναι ο $$2$$, άρα πρέπει να ελέγξουμε όλα τα $$3$$ για κάθε $$6$$ (μέχρι που να δούμε ότι το $$5$$ δεν διαιρεί το $$6$$). Αλλά για τα $$A_i = 3$$ έχουμε βρει έναν ακέραιο $$A_j = 6$$ (και $$A_j = 3$$) με $$j > i$$ ώστε $$A_i$$ διαιρεί τον $$A_j$$, άρα για τους επόμενους ακεραίους αρκεί να κοιτάξουμε ότι ο $$A_j$$ τους διαιρεί (και αυτό συνεπάγεται ότι και ο $$A_i$$ τους διαιρεί). Πιο συγκεκριμένα, επεκτίνουμε την **Παρατήρηση 1** ως εξής:
 ...
 * κρατάμε τον δείκτη `tested_up_to` στο τελευταίο στοιχείο που βρήκαμε κάποιο δεξιότερά του που να το διαιρεί (δλδ `Α[tested_up_to]` διαιρεί κάποιο `A[j]` για `i >= j > tested_up_to` και δεν ισχύει για το `tested_up_to+1`).
 * Για να ελέγξουμε το στοιχείο `i` προχωράμε τον δείκτη `tested_up_to` έως ότου είτε να φτάσουμε το `i` είτε βρούμε ένα στοιχείο που δεν διαιρεί το `A[i]`.
...
Για κάθε στοιχείο πρέπει να καλέσουμε μία φορά τη συνάρτηση `lcm`, άρα είναι η πολυπλοκότητα είναι $$\mathcal{O}(N \cdot \log{(\mathrm{MAX\_LCM})}) = \mathcal{O}(N \cdot \log{(2 \cdot 10^9)})$$. Παρατηρούμε ότι κάθε στιγμή χρειάζομαι μόνο τον `A[i]`, `cur_max` και το `cur_lcm`, επομένως η μνήμη είναι $$\mathcal{O}(1)$$.
