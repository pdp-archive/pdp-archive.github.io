...
Ένας τρόπος να ελέγξουμε αν ο γράφος είναι συνδεδεμένος είναι κάνοντας αναζήτηση κατά βάθος (DFS). Ο τρόπος που δουλεύει η μη-αναδρομική DFS είναι διατηρώντας μίσ στοίβα $$\textit{st}$$ και για κάθε κορυφή $$v$$ το $$visit[v]$$, που μας λέει αν έχουμε επισκεφθεί την κορυφή $$v$$. Ξεκινάμε με την στοίβα να έχει ένα οποιοδήποτε κορυφή. Έπειτα όσο η στοίβα δεν είναι άδεια, αφαιρεί την πάνω πάνω κορυφή και (αν δεν την έχουμε επισκεφθεί) προσθέτει τους γείτονές της στη στοίβα. 
...
Ο έλεγχος για το αν ο γράφος είναι συνδεδεμένος θέλει $$\mathcal{O}(E + V)$$ χρόνο και χρειάζεται να τον κάνουμε το πολύ για κάθε ακμή. Άρα συνολικά ο αλγόριθμος χρειάζεται $$\mathcal{O}(E \cdot (V + E))$$ χρόνο. Αυτό είναι αρκετό για να περάσει περίπου 14-16 από τα 21 testcases. 
...
Εμείς θέλουμε να βρούμε το πρώτο σημείο που συνάρτηση επιστρέφει `true`. Επομένως μπορούμε να χρησιμοποιήσουμε δυαδική αναζήτηση:
...
**Θεωρητική σημείωση:** Ο αλγόριθμος αυτός είναι ίδιος με αυτόν του Kruskal για την εύρεση του ελάχιστου συνδετικού δένδρου (minimum spanning tree (mst)). Ισχύει η απάντηση σε αυτό το πρόβλημα είναι η μεγαλύτερη ακμή σε ένα (οποιδήποτε) mst. Επομένως θα μπορούσαμε να χρησιμοποιήσουμε οποινδήποτε άλλον αλγόριθμο για την εύρεση του mst (πχ του Prim). 
...
Ο αλγόριθμος στηρίζεται ότι μπορούμε να βρούμε το μεσαίο όρο μίας ακολουθίας από $$N$$ όρους σε $$\mathcal{O}(N)$$ χρόνο. Ξεκινάμε βρίσκοντας την μεσαίου βάρους $$w_m$$ ακμή. Προσθέτουμε όλες τις $$E/2$$ ακμές με βάρος μικρότερες ή ίσες από αυτήν στον γράφο (σε $$\mathcal{O}(E)$$ χρόνο). Αν ο γράφος είναι συνδεδεμένος ξέρουμε ότι η απάντηση είναι στις $$E/2$$ μικρότερες, όποτε συνεχίζουμε αναδρομικά. Αν ο γράφος δεν είναι συνδεδεμένος, η απάντηση είναι στις $$E/2$$ μεγαλύτερες, οπότε ενώνουμε τις κορυφές συνδεδεμένες με τις $$E/2$$ μικρότερες ακμές και συνεχίζουμε αναδρομικά.

Αν υλοποιήσουμε με προσοχή το κάθε βήμα ώστε να θέλει χρόνο γραμμικό προς το πλήθος των ακμών στον γράφο, τότε η συνολική πολυπλοκότητα είναι $$\mathcal{O}(E + E/2 + E/4 + \ldots + 1) = \mathcal{O}(E)$$. Μπορείτε να βρείτε [εδώ]({% include link_to_source.md solution_name='smalltank_linear.cc' %}) ολόκληρο τον κώδικα.
