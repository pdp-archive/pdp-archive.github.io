...
Μας δίνεται ότι ένας συνδεδεμένος μη-κατευθυνόμενος γράφος που έχει την μορφή ενός δέντρου + μία ακμή. Μας ζητείται για κάθε ακμή του γράφου να βρούμε τον μήκος του μακρύτερου μονοπατιού μεταξύ δύο κορυφών του γράφου αν αυτή αφαιρεθεί από τον γράφο.
...
Ο αλγόριθμος αυτός χρειάζεται $$O(N^2)$$ για κάθε ακμή, άρα $$O(N^3)$$ συνολικά.

Αν χρησιμοποιήσουμε ότι ο υπολοιπόμενος γράφος είναι ένα δέντρο, τότε μπορούμε να βρούμε το μακρύτερο μονοπάτι σε $$\mathcal{O}(N)$$ χρόνο, κάνοντας δύο αναζητήσεις κατά βάθος: 
...
Σε αυτό το υποπρόβλημα υπάρχουν μόνο τρεις κόμβοι στον κύκλο, άρα μπορούμε απλά να τους αφαιρέσουμε και να βρούμε το μακρύτερο μονοπάτι στο υπολοιπόμενο δέντρο σε συνολικό χρόνο $$O(N)$$ (δείτε τον κώδικα [εδώ]({% include link_to_source.md solution_name='subtask3.cc' %})). 


## Υποπρόβλημα 4

Εδώ υπάρχει ένα σχετικά μικρό πλήθος κόμβων στον κύκλο. Όταν αφαιρούμε μία ακμή από τον κύκλο υπάρχουν τα εξής πιθανά σενάρια:
...
Για την δεύτερη περίπτωση, αφαιρούμε την ακμή από τον κύκλο και τον αντιμετωπίζουμε σαν έναν πίνακα. Βρίσκουμε $$w[i]$$ το βάθος του δέντρο στον κόμβο $$i$$ και έπειτα ψάχνουμε να βρούμε τους δύο κόμβους στον πίνακα με το μέγιστο $$w[i] + w[j] + \lvert i - j\rvert$$.
...
Κάθε ερώτημα χρειάζεται $$O(C^2 )$$ χρόνο, όπου $$C$$ είναι το μήκος του κύκλου. Συνεπώς, συνολικά χρειάζεται $$O(NC^2)$$ χρόνο που είναι αρκετό για να περάσει το υποπρόβλημα. 

## Υποπρόβλημα 5

Εδώ το μήκος του κύκλου είναι λίγο πιο μεγάλο και θα προσπαθήσουμε να απαντήσουμε το κάθε ερώτημα σε $$O(C)$$ χρόνο. Η βασική παρατήρηση που κάνουμε είναι η εξής:
...
Κάθε ερώτημα χρειάζεται $$O(C)$$ χρόνο και συνολικά $$O(NC^2)$$ χρόνο που είναι αρκετό για να περάσει το υποπρόβλημα. Ολόκληρος ο κώδικας [εδώ]({% include link_to_source.md solution_name='subtask5.cc' %}).
...
Για να το κάνουμε αυτό αποδοτικά κρατάμε ένα set με τα στοιχεία ``{ w[i] + i, i }`` και ένα με τα στοιχεία ``{ w[j] - j, j }``. Σε κάθε βήμα, αφαιρούμε τα στοιχεία που είναι παλιά (δηλαδή έχουν $i$ με $$i \leq k - C$$) και θέλουμε να βρούμε το μέγιστο άθροισμα αυτών των δύο. Υπάρχουν οι εξής δύο περιπτώσεις:
 ...
 2. Είναι το άθροισμα από το ένα μέγιστο και το δεύτερο μέγλύτερο από το άλλο σύνολο.
 
Το set μας επιτρέπει να κάνουμε αυτές τις πράξεις σε $$O(\log C)$$ χρόνο. 

{% include code.md solution_name='efficient.cc' start=112 end=137 %}

Συνολικά ο αλγόριθμος χρειάζεται $$O(N \log N)$$ χρόνο και είναι αρκετό για να περάσει όλα τα testcases. 
