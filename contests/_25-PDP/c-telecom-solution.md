...
Για να βρούμε το μεγαλύτερο βάρος στο μονοπάτι μεταξύ $$i$$ και $$j$$ κάνουμε αναζήτηση κατά βάθος. Υπάρχουν $$N(N-1)/2 - (N-1) = \mathcal{O}(N^2)$$ ακμές και κάθε DFS θέλει $$\mathcal{O}(N)$$ χρόνο, άρα συνολικά ο αλγόριθμος θέλει $$\mathcal{O}(N^3)$$ και $$\mathcal{O}(N)$$ μνήμη. 
...
Συνεπώς, μπορούμε να βρούμε το άθροισμα όλων των μέγιστων αποστάσεων σε χρόνο $$\mathcal{O}(N^2)$$ (αφού για κάθε κορυφή χρειάζεται $$\mathcal{O}(N)$$) και με $$\mathcal{O}(N)$$ μνήμη.
...
Ο αλγόριθμος αυτός έχει πολυπλοκότητα $$\mathcal{O}(N\alpha(N) + N\log{N})$$ ή $$\mathcal{O}(N\alpha(N))$$ (αν η ταξινόμηση γίνει γραμμικά). 
