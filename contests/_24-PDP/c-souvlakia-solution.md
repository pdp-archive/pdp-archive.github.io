...
να βρούμε τις αποστάσεις αυτές. Οι τιμές των αποστάσεων αυτών $$x,y,z \in [0, D \cdot N]$$ όπου $$D \leq 2 \cdot 10^4$$ το μέγιστο βάρος κάθε ακμής. 
...
Προσέξτε ότι δεν είναι η $$\mathcal{O}(N^2)$$ η χειρότερη λύση, διότι αν δεν είχαμε επιλέξει μια λογαριθμική λύση με τη χρήση του __std::set__ 
για το Dijkstra αλλά την μια λύση με loop $$\mathcal{O}((N+M)\cdot N)$$ όπου το μέγιστο 
...
να μπορέσουμε να αφαιρέσουμε τουλάχιστο τις δύο συνθήκες με κάποιο έξυπνο τρόπο ώστε να μας μείνει μια γραμμική προσπέλαση για την τρίτη. 
Εφόσον έχουμε να βρούμε διάταξη μεταξύ των αντίτστοιχων τιμών, μπορούμε να αφαιρέσουμε την μια από τις τρεις συνθήκες αντικαθιστώντας την με 
...
με $$x_j\lt x_i$$. Αυτό που καταφέρνουμε είναι ότι όλα τα καταστήματα που μπορούν να βγάλουν το κατάστημα $$i$$ ως μη επιλέξιμο, θα τα έχουμε 
...
$$i$$ δεν είναι επιλέξιμο. Η δομή αυτή που ζητάμε θα πρέπει να μας απαντά το ελάχιστο $$z$$ για όλα $$y \in [0,y_i)$$ 
...
νέα ζεύγη $$(y_i,z_i)$$ για κάθε κατάστημα $$i$$ της ομάδας καθώς αυτά ενδέχετε να επηρεάζουν τα καταστήματα με μεγαλύτερα $$x$$ που 
θα επεξεργαστούμε στη συνέχεια.
...
Μία ακόμα ενδεικτική λύση με την εξαιρετικά γρήγορη και μικρή σε κώδικα και απαιτούμενη μνήμη (σε σχέση με το Segment Tree) είναι η δομή Binary Index Tree. 
Το Binary Index Tree παρόλο που και αυτό έχει λογαριθμική πολυπλοκότητα είναι φανερό ότι απαιτει πολύ λιγότερες εντολές για να κάνει Range Queries. Το 
μειονέκτημα του είναι ότι δεν είναι τόσο γενική χρήσης (δεν έχει το ίδιο πεδίο εφαρμογών) με το Segment Tree.
Για να δούμε τη λύση με Binary Index Tree αρκει να αντικαταστήσουμε τον κώδικα του Segment Tree με το παρακάτω. 
...
κάποιο node έχει δεξιό άκρο το $$y_i$$ δεν υπάρχει λόγος να συνεχίσουμε να κάνουμε expand σε μεγαλύτερο βάθος, διότι θα έχει οριοθετήσει ικανοποιητικά 
το range $$[0,y_i]$$ και τα queries με αυτό το $$y_i$$ θα βρίσκουν την απάντηση τους στον κόμβο αυτό χωρίς να χρειάζονται να συνεχίζουν σε μεγαλύτερο βάθος. 

Η λύση μας χρησιμοποιεί το __implicit segment tree__ το οποίο γενικά χρησιμοποιείται όταν ξέρουμε από την αρχή τις 
τιμές όλων των κόμβων ή όταν αυτές εννοούνται (implied). Παράδειγμα δεν έχει νόημα να κάνουμε build ένα segment tree που οι τιμές όλων των κόμβων είναι $$0$$.
Στην περίπτωση μας όλοι οι κόμβοι εννοείται ότι είναι $$\mathit{INF}$$ ώστε να μην επηρεάζουν κανένα κατάστημα και σταδιακά θα αρχίσουν να γίνονται update. 
...
αν χρειαστούν και χρησιμοποιούνται pointers για τη διαχείριση του tree) παρακάτω. Η χρήση pointers και dynamic memory allocation θα 
καταλάβει μόνο όση μνήμη χρειάζεται αλλά η χρήση pointers απαιτεί μεγαλύτερη ευχέρεια στο χειρισμό της γλώσσας προγραμματισμού.
...
Κάθε query στη θέση $$y_i$$ έχει σαν απάντηση το $$\mathit{RMQ}[0..y_i]$$ δηλαδή το
...
φορά που θα εισάγουμε ένα νέο ζεύγος $$(y_i,z_i)$$ ή θα βελτιώνουμε ένα υπάρχον ζεύγος με μικρότερο $$z_i$$. Καθώς εισάγουμε ή μεταβάλουμε ένα 
σημείο της ακολουθίας μας διορθώνουμε (διαγράφουμε) τα ασύμβατα στοιχεία εκατέρωθεν του $$y_i$$ ώστε η ακολουθία μας να παραμείνει γνησίως φθίνουσα. 
Υπάρχει ενδεχόμενο φυσικά να διαγραφεί και το ζεύγος που εισάγαμε αν δεν ήταν συμβατό με τη μονοτονία της ακολουθίας μας μέχρι τώρα.   

Κάθε κατάστημα λοιπόν θα εισάγει ένα ζεύγος στοιχείων και κάθε εισαγωγή ελέγχει και διαγράφει $$0$$ έως $$N$$ στοιχεία για να διατηρήσει
...
{% include code.md solution_name='souvlakia_nlogn_map.cc' start=64 end=78 %}

Υπάρχουν μερικές υλοποιήσεις στο διαδίκτυο που κάνουν τον πολλαπλασιασμό με το $$10$$ σε μια πρόσθεση και δύο ολισθήσεις
```c++
	x = x<<1 + x<<3 + c;
	//x<<1 is equivalent to pow(x,1) and x<<3 equivalent to pow(2,3)
```
αντί για 
```c++
	x = x*10 + c;
```
δεν υπάρχει κανένας λόγος να γίνει αυτό καθότι ο πολλαπλασιασμός ακεραίων με μέγεθος όσο ο καταχωρητής συσσώρευσης ACCumulator 
του επεξεργαστή γίνεται σε ίδιο χρόνο με την πρόσθεση στους σύγχρονους υπολογιστές από την ALU (Arithmetic and Logic Unit) εδώ 
και τουλάχιστο μια δεκαετία. Οπότε η εντολή με τις ολισθήσεις είναι ελάχιστα πιο αργή (μη μετρήσιμο όμως) διότι έχει $$3$$ πράξεις 
και μια απόδοση τιμής ενώ η δεύτερη $$2$$ πράξεις και μια απόδοση τιμής.
