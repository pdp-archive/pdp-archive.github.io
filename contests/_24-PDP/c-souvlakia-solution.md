...
Η συνολική πολυπλοκότητα περιορίζεται από τον χειρότερο όρο άρα είναι της τάξης $$\mathcal{O}(N^2)$$<sup>1</sup>.
...
$$i$$ δεν είναι επιλέξιμο γιατί τουλάχιστο ένα άλλο κατάστημα προτιμάται. Η δομή αυτή που ζητάμε θα πρέπει να μας απαντά το ελάχιστο $$z$$ για όλα $$y \in [0,y_i)$$ 
...
και ταχύτερο Binary Index Tree θα ήθελε μνήμη της τάξης $$\mathcal{O}(D\cdot N)$$ (όπου $$D$$ το μέγιστο βάρος μιας edge του γράφου), ενώ 
...
νέα ζεύγη $$(y_i,z_i)$$ για κάθε κατάστημα $$i$$ της ομάδας ώστε να ετοιμάσουμε τη δομή μας για τον έλεγχο των επόμενων ομάδων που ενδέχετε να επηρεάζονται 
από την ομάδα που μόλις προσπελάσαμε.
...
Ο παραπάνω πλήρης κώδικας χωρίστηκε σε τρία τμήματα, το πρώτο περιέχει σταθερές, το τμήμα του Dijkstra και μερικές βοηθητικές i/o συναρτήσεις για fast-io<sup>2</sup>,
...
είναι η δομή Binary Index Tree.  
Το Binary Index Tree παρόλο που και αυτό έχει λογαριθμική πολυπλοκότητα είναι φανερό ότι απαιτει πολύ λιγότερες 
...
Το module του Binary Index Tree που αντικαθιστα το module του segment tree στον παραπάνω κώδικα ακολουθεί:
...
<sup>1</sup> Η $$\mathcal{O}(N^2)$$ δεν είναι η χειρότερη λύση, διότι αν δεν είχαμε επιλέξει μια λογαριθμική λύση με τη χρήση του __std::set__ 
για το Dijkstra αλλά μια λύση με loop $$\mathcal{O}((N+M)\cdot N)$$ όπου στο συγκεκριμένο πρόβλημα το μέγιστο 
$$M$$ είναι πολλαπλάσιο του $$N$$ η λύση μας θα ήταν ακόμα χειρότερη.

<sup>2</sup> Τα στοιχεία στο input είναι πάρα πολλά (ακμές γράφων και ερωτήματα) και ο χρόνος που απαιτείται για το input είναι 
...
```c++
	x = x<<1 + x<<3 + (c-'0');
	//x<<1 is equivalent to x*pow(2,1) and x<<3 equivalent to x*pow(2,3)
	//so the above expr is equivalent to x = 2*x + 8*x + (c-'0')
```
αντί για 
```c++
	x = x*10 + (c-'0');
...
<sup>3</sup> Οι ακμές των γράφων δεν είναι χρήσιμες μετά την εκτέλεση του Dijkstra οπότε μπορούμε να χρησιμοποιήσουμε 
__vector__ τοπικά μέσα σε compound statement ώστε όταν τελειώσει η compound statement να ελευθερωθεί η μνήμη.  
παράδειγμα:
```c++
	{//compound statement
	   vector<int> K(10007);//δημιουργήθηκε ένας δυναμικός πίνακας 10007 ακεραίων 
	   //...κάνε κάτι με τον πίνακα...
	}//<---ο πίνακας K έγινε destruct, η μνήμη του ελευθερώθηκε και αν δεν δημιουργείτε πρόβλημα
	 //κατακερματισμού μνήμης, αυτή μπορεί να επαναχρησιμοποιηθεί.
```

<sup>4</sup> Ο Dijkstra μπορεί να υπερχειλίσει κάποιους τύπους ακεραίων όταν πάει να προσθέσει δυο $$\mathit{INF}$$ μεταξύ τους οπότε 
