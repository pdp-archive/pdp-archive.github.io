...
τις αποστάσεις δηλαδή από τα τρία προεπιλεγμένα καταστήματα οπότε μπορούμε να συνεχίσουμε στο κυρίως πρόβλημα μας του οποίου η επεξήγηση ακολουθεί.
...
## Ικανοποιητική λύση - $$\mathcal{O}(N \cdot log(D\cdot N))$$
...
## Βέλτιστη λύση Νο 1 - $$\mathcal{O}(N \cdot log{N})$$

Γνώσεις που θα χρειαστούμε: σύμπτυξη ή συμπίεση συντεταγμένων (coordinate compression)

Γνωρίζοντας ότι όλες οι $$y_i$$ τιμές είναι το πολύ $$N$$, μπορούμε να τις "συμπιέσουμε", δηλαδή να τις αντικαταστήσουμε με **διαδοχικούς** αριθμούς τέτοιους (αφαιρώντας τα "κενά")  ώστε να ισχύουν οι παρακάτω συνθήκες  
...
Δηλαδή αν υπάρχουν $$\mathit{YMAX}$$ διαφορετικές τιμές από το σύνολο όλων των τιμών $$y$$ των καταστημάτων, αντικατέστησε το κάθε $$y_i$$ κάθε καταστήματος $$i$$ με έναν αριθμό από το $$1$$ έως το $$\mathit{YMAX}$$  (συμπιεσμένη τιμή) έτσι ώστε να μην χαλάσει η σειρά (order) μεταξύ των καταστημάτων. Με τον τρόπο αυτό περιορίζουμε το πεδίο ορισμού που πρέπει να απαντά ερωτήματα το Binary Index Tree ή το Segment Tree από $$D\cdot N$$ σε $$\mathit{YMAX}$$, οπότε δεν ξεπερνάμε τα όρια μνήμης του προβλήματος καθώς ισχύει ότι  

$$0\lt\mathit{YMAX}\leq N < D\cdot N$$  

Ενδεικτική λύση ακολουθεί με χρήση της δομής Binary Index Tree. Η λύση αυτή περνά όλα τα test cases.
...
## Βέλτιστη λύση Νο 2 - $$\mathcal{O}(N \cdot log{N})$$
...
Αυτό που θέλουμε δηλαδή είναι ένα **prefix minimum** η δημιουργία του οποίου γενικά παίρνει χρόνο $$\mathcal{O}(N)$$ (όπως και του prefix sum).
...
$$\forall i,j \in [1,N], y_i \lt y_j \Leftrightarrow \mathit{RMQ}[0..y_i] \gt \mathit{RMQ}[0..y_j]$$  
