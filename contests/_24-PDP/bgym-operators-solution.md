...
Ένας τρόπος να λύσουμε αυτό το πρόβλημα είναι δοκιμάζοντας όλα τα πιθανά ζεύγη θέσεων της ακολουθίας, την οποία μπορούμε να έχουμε αποθηκεύσει σε έναν πίνακα, έστω $$A$$. Αυτό μπορεί να γίνει με δύο εμφωλευμένα for loops, έστω με μεταβλητές $$i$$ και $$j$$. Αρκεί να εξετάσουμε μόνο τις περιπτώσεις όπου $$i < j$$, αφού οι υπόλοιπες αποτελούν επαναλήψεις τους. Μπορούμε στη μεταβλητή $$mn$$ να διατηρούμε το ελάχιστο μέτρο αθροίσματος που έχουμε συναντήσει μέχρι στιγμής, ενώ στις μεταβλητές $$mn\_i$$ και $$mn\_j$$ μπορούμε να αποθηκεύουμε τις θέσεις των ακεραίων που παρήγαγαν το ελάχιστο αυτό μέτρο αθροίσματος. Η μεταβλητή $$mn$$ μπορεί να αρχικοποιηθεί σε $$2 \cdot 10^9 + 1$$ ώστε σύμφωνα με τους περιορισμούς εισόδου να ενημερωθεί στην πρώτη κιόλας επανάληψη. Γενικά η μεταβλητή θα ενημερώνεται όταν βρίσκουμε μικρότερο μέτρο αθροίσματος, δηλαδή όταν $$|A[i] + A[j]| < mn$$, οπότε θα θέτουμε και $$mn\_i = i$$ και $$mn\_j = j$$. Το αποτέλεσμα που θα εκτυπώσουμε θα είναι οι ακέραιοι της ακολουθίας $$A[mn\_i]$$ και $$A[mn\_j]$$ με τη σειρά αυτή.
...
Ο εντοπισμός του αντιθέτου ή των γειτόνων του για κάθε μη μηδενικό ακέραιο, έστω $$K$$, μπορεί να γίνει με δυαδική αναζήτηση πάνω στη συνθήκη $$A[i] >= -K$$ με το $$i$$ να είναι η μεταβλητή της δυαδικής αναζήτησης και να παίρνει τιμές στο διάστημα ακεραίων $$[0, N - 1]$$. Αυτή μπορεί να γίνει εύκολα με χρήση της συνάρτησης lower_bound από τη βιβλιοθήκη **algorithm**. Βέβαια απαιτείται και ο έλεγχος για το εάν έχουμε $$1$$ ή $$2$$ γείτονες. Να σημειώσουμε ότι η δυαδικές αναζητήσεις δεν απαιτούν ταξινόμηση της ακολουθίας, αφού αυτή είναι ήδη αύξουσα.

Ο αλγόρθμος αυτός πραγματοποιεί $$O(N)$$ δυαδικές αναζητήσεις κάθε μία από τις οποίες απαιτεί χρόνο $$O(logN)$$, επομένως η πολυπλοκότητα του αλγρίθμου είναι $$O(NlogN)$$. Η λύση αυτή είναι αρκετά γρήγορη για να περάσει όλα τα testcases. Η χωρική πολυπλοκότητα είναι $$O(N)$$.
...
**Παρατήρηση**: Αν σε μία φάση της εκτέλεσης η μεταβλητή $$j$$ έχει επιλεγεί κατάλληλα ώστε η τιμή $$|A[i] + A[j]|$$ να είναι ελάχιστη για δεδομένο $$i$$, τότε αν αυξήσουμε το $$i$$ το αποτέλεσμα δε θα μπορεί να βελτιωθεί με αύξηση και του $$j$$.
...
* Αν $$A[i] + A[j] >= 0$$ και το $$i$$ αυξηθεί, τότε δε συμφέρει να αυξηθεί και το $$j$$ καθώς η τιμή του $$A[i] + A[j]$$ και άρα και του $$|A[i] + A[j]|$$ δε θα μπορεί να μειωθεί.
* Αν $$A[i] + A[j] < 0$$ τότε $$A[i] + A[j + 1] > 0$$ και $$|A[i] + A[j + 1]| >= |A[i] + A[j]|$$ διαφορετικά το $$j$$ δε θα είχε βέλτιστη τιμή όπως υποθέσαμε. ’ρα αν το $$i$$ αυξηθεί δε θα συμφέρει να αυξηθεί και το $$j$$ καθώς κάτι τέτοιο δε θα μπορούσε να οδηγήσει σε μείωση του μέτρου του αθροίσματος.
