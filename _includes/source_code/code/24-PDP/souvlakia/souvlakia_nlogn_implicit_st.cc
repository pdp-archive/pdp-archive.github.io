...
long stn;//keep track of ST[] pool usage
const long SMAX = MAXN * 30;//log2(MAXN * D) == 30
const long YMAX = long(2e4)*MAXN;	//max value of Y

struct node {//implicit segment tree (lazy build)
	long left,right;//Y limits that this node handles
	long minZ;//min Shop.Z value of segment tree
	long leftptr,rightptr;//"pointers" to the children 
...
void supdate(long y,long z,long root = 0){
	node& s = st[root];	
	s.minZ = min(s.minZ,z);
	if(s.right == y)
