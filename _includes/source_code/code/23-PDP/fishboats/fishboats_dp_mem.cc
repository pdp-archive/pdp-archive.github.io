   ...
   for (int total_boats = 1; total_boats <= N; ++total_boats) {
      ...
      for (int i = 0; i < left.size(); ++i) {
         ...
         for (int j = 0; j < mx_j; ++j) {
            ...
            dp[md(i)][j][0] = dp[md(i)][j][1] = -INF;
            if (i >= 1) dp[md(i)][j][0] = M + std::max(
               dp[md(i-1)][j][0] - remaining_boats * (left[i] - left[i-1]),
               dp[md(i-1)][j][1] - remaining_boats * (left[i] + right[j]));
            if (j >= 1) dp[md(i)][j][1] = M + std::max(
               dp[md(i)][j-1][1] - remaining_boats * (right[j] - right[j-1]),
               dp[md(i)][j-1][0] - remaining_boats * (left[i] + right[j]));
            // Η λύση είναι πιθανή μόνο όταν i+j == total_boats.
            if (i + j == total_boats) cur_best = std::max(
               cur_best, 
               std::max(dp[md(i)][j][0], dp[md(i)][j][1]));
         }
      }
   }
